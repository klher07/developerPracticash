...
//AQUI CONFIRMMO SI EL NUMERO DE TLF ES DE PABLO
  		...
  		if($numero_suscribir1 == '5804268210636' || $numero_suscribir1 == '584268210636')
  		{
			$sid1 = 'ACe204ca84c384c61b2bd93e34b2c3ff8b';
			$token1 = 'f092612038e50ee2a558b1695e273d5d';
...
//AQUI CONFIRMMO SI EL NUMERO DE TLF ES DE MIGUEL
  		if($numero_suscribir1 == '5804122111063'  || $numero_suscribir1 == '584122111063')
  		{
			$sid1 = 'AC43f2c59733cff635f0fe4d18ad35b36d';
			$token1 = 'adb272694a0765aaa259a22c42ef4321';
			$telefono_twilio = '+15134404286';  			
  		}
...
//aqui registra en transaccion que tiene un saldo de 0
			...
			$sql_registro_t ="INSERT INTO transacciones(fecha,id_cliente,valor,descripcion,observacion,id_tarjeta, stripe_id,transaccion_id) VALUES('$f_tran','$id_cliente_n',0,'Apertura de Cuenta Practicash','Bienvenido',1,'1','1')";
...
//echo json_encode($tabla_total3);
			...
			if ($df ==0)
			{
				...
				//aqui se agregan los saldos calculados a la tabla transaccion
					$sql_transaccion2 = "SELECT t.id, t.fecha, t.descripcion, t.valor,t.observacion, c.nombre_cliente, t.info_contraparte, t.id_cliente AS saldo FROM transacciones t INNER JOIN clientes c ON t.id_cliente = c.id WHERE t.id_cliente = '$id_cli_transa' ORDER BY t.id DESC";
	...
	{
		if(isset($_POST['telefono_paga']) && isset($_POST['pin_pago']) && isset($_POST['valor_de_pago']) && isset($_POST['telefono_recibe']))
	    {

			$telefono_paga = $_POST['telefono_paga'];
			$pin_pago = $_POST['pin_pago'];
...
//aqui se valida si el usuario a pagar existe en el sistema
		...
		//aqui se obtiene el id del cliente que paga
			...
			//aqui se inserta el registro de recibe dinero
				...
				//aqui se consulta el nombre de la contraparte receptora del pago
				$sql_nombre_contraparte_re = "SELECT nombre_cliente FROM clientes WHERE telefono = '$telefono_paga'";
				
				$resultado_sql_nombre_contraparte_re=pg_query($cone,$sql_nombre_contraparte_re);
				
				$fila_contraparte_re=pg_fetch_assoc($resultado_sql_nombre_contraparte_re);
				
				$nombre_contraparte_re = $telefono_paga." - ".$fila_contraparte_re['nombre_cliente'];

				$sql_cliente_recibe = "INSERT INTO transacciones(fecha,id_cliente,valor,descripcion,observacion,id_tarjeta, stripe_id,transaccion_id, info_contraparte) VALUES('$fecha_recibe','$id_cli_re','$valor_de_pago','$descripcion_cli_re','$concepto','$id_tarjeta_recibe','$stripe_id','$valor_tem_re', '$nombre_contraparte_re') 
			";
			...
			//aqui se inserta el registro de pagar dinero
				...
				//aqui se consulta el nombre de la contraparte del pago
				$sql_nombre_contraparte_pa = "SELECT nombre_cliente FROM clientes WHERE telefono = '$telefono_recibe'";
				
				$resultado_sql_nombre_contraparte_pa=pg_query($cone,$sql_nombre_contraparte_pa);
				
				$fila_contraparte_pa=pg_fetch_assoc($resultado_sql_nombre_contraparte_pa);
				
				$nombre_contraparte = $telefono_recibe." - ".$fila_contraparte_pa['nombre_cliente'];
				
				$sql_cliente_paga = "INSERT INTO transacciones(fecha,id_cliente,valor,descripcion,observacion,id_tarjeta, stripe_id,transaccion_id, info_contraparte) VALUES('$fecha_paga','$id_cli_pa','-$valor_de_pago','$descripcion_cli_pa','$concepto','$id_tarjeta_paga','$stripe_id_paga','$valor_tem_paga', '$nombre_contraparte')";
